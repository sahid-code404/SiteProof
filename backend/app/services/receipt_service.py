...
AUTONOMOUS_RECEIPT_SCHEMA_VERSION = "1.1"
    ...
    return [
        {
            ...
            "score": fixed_decimal(row.score, 6),
            "confidence": fixed_decimal(row.confidence, 6),
            "weightedContribution": fixed_decimal(row.weighted_contribution, 6),
...
def _optional_decimal(value: float | None, places: int = 6) -> str | None:
    return fixed_decimal(value, places) if value is not None else None


def _autonomous_receipt_binding(
    db: Session,
    result: VerificationResult,
) -> dict[str, object] | None:
    """Build a signed digest binding for every semantic artifact that influenced engine v3.

    The receipt intentionally does not expose the full admin contract or model observations. It
    commits to them by SHA-256, while also preserving the model/version identifiers, sampled frame
    hashes and normalized decision metrics necessary to audit which autonomous evidence governed
    the result.
    """
    if result.engine_version != AUTONOMOUS_ENGINE_VERSION:
        return None

    autonomous = get_autonomous_result(db, result.session_id)
    if autonomous is None or autonomous.status != AutonomousAnalysisStatus.COMPLETE:
        raise SiteProofError(
            409,
            "AUTONOMOUS_EVIDENCE_NOT_COMPLETE",
            "An autonomous verification receipt requires completed semantic evidence analysis.",
        )

    binding: dict[str, object] = {
        "analysisVersion": autonomous.analysis_version,
        "contractVersion": autonomous.contract_version,
        "contractPromptVersion": autonomous.contract_prompt_version,
        "visionPromptVersion": autonomous.vision_prompt_version,
        "compilerModel": autonomous.compiler_model,
        "primaryVlmModel": autonomous.primary_vlm_model,
        "secondaryVlmModel": autonomous.secondary_vlm_model,
        "contractSourceHash": autonomous.contract_source_hash,
        "contractSha256": sha256_hex(canonical_json_bytes(autonomous.contract_json or {})),
        "observationsSha256": sha256_hex(
            canonical_json_bytes(autonomous.observations_json or {})
        ),
        "rawResponseHashes": autonomous.raw_response_hashes_json or {},
        "sampledFrameCount": autonomous.sampled_frame_count,
        "frameHashes": autonomous.frame_hashes_json or [],
        "taskMatch": {
            "score": _optional_decimal(autonomous.task_match_score),
            "confidence": _optional_decimal(autonomous.task_match_confidence),
        },
        "assetIdentity": {
            "score": _optional_decimal(autonomous.asset_identity_score),
            "confidence": _optional_decimal(autonomous.asset_identity_confidence),
        },
        "evidenceCoverage": {
            "score": _optional_decimal(autonomous.evidence_coverage_score),
            "confidence": _optional_decimal(autonomous.evidence_coverage_confidence),
        },
        "liveScene": {
            "score": _optional_decimal(autonomous.live_scene_score),
            "confidence": _optional_decimal(autonomous.live_scene_confidence),
        },
        "presentationAttack": {
            "score": _optional_decimal(autonomous.presentation_attack_score),
            "confidence": _optional_decimal(autonomous.presentation_attack_confidence),
        },
        "mandatoryFailureCount": len(autonomous.mandatory_failures_json or []),
        "modelDisagreement": autonomous.model_disagreement,
        "analyzedAt": autonomous.analyzed_at,
    }
    binding["bindingSha256"] = sha256_hex(canonical_json_bytes(binding))
    return binding


def ensure_key_metadata(
    db: Session,
    provider: SigningKeyProvider,
    *,
    now: datetime | None = None,
) -> tuple[SigningKey, list[SigningKey]]:
    public = provider.get_public_key()
    instant = now or utc_now()
    existing = db.scalar(select(SigningKey).where(SigningKey.key_id == public.key_id))
    if existing is not None:
        if (
            existing.algorithm != public.algorithm
            or existing.public_key_base64 != public.public_key_base64
        ):
            raise SiteProofError(
                409,
                "SIGNING_KEY_ID_CONFLICT",
                "Signing key ID already refers to different public key material.",
            )
        if existing.status != SigningKeyStatus.ACTIVE:
            raise SiteProofError(
                409,
                "SIGNING_KEY_NOT_ACTIVE",
                "Retired or compromised keys cannot issue new receipts.",
            )
        return existing, []
    retired = list(
        db.scalars(select(SigningKey).where(SigningKey.status == SigningKeyStatus.ACTIVE)).all()
    )
    for old in retired:
    ...
    row = SigningKey(
        ...
        external_reference="external-secret",
        active_from=instant,
    )
    db.add(row)
    db.flush()
...
def issue_automated_receipt(
    ...
    *,
    ...
    if (
        ...
        or result.overall_confidence is None
        or result.verdict is None
    ):
        raise SiteProofError(
            409,
            "VERIFICATION_NOT_COMPLETE",
            "A receipt can only seal a completed verification result.",
        )
    existing = db.scalar(
        select(SignedReceipt).where(
            SignedReceipt.verification_result_id == result.id,
            SignedReceipt.receipt_type == ReceiptType.AUTOMATED_VERIFICATION,
        )
    )
    if existing is not None:
        return existing
    ...
    autonomous_binding = _autonomous_receipt_binding(db, result)
    schema_version = (
        AUTONOMOUS_RECEIPT_SCHEMA_VERSION
    ...
    for retired in retired_keys:
        record_audit(
            ...
            metadata={"retiredKeyId": retired.key_id, "activeKeyId": key.key_id},
        )
    record_audit(
        db,
        organization_id=result.organization_id,
        ...
        metadata={"receiptType": ReceiptType.AUTOMATED_VERIFICATION.value},
    )
    ...
    verification_payload: dict[str, object] = {
        "score": fixed_decimal(result.final_score, 2),
        "confidence": fixed_decimal(result.overall_confidence, 6),
        "verdict": result.verdict.value,
        "policyId": str(result.policy_id),
        "policyName": result.policy_name,
        "policyVersion": result.policy_version,
        "engineVersion": result.engine_version,
        "signals": _signal_summary(db, result.id),
    }
    if autonomous_binding is not None:
        verification_payload["autonomous"] = autonomous_binding

    payload = {
        "schemaVersion": schema_version,
        "receiptId": str(receipt_id),
        "receiptNumber": receipt_number,
        "receiptType": ReceiptType.AUTOMATED_VERIFICATION.value,
        "organizationId": str(result.organization_id),
        "inspectionId": str(result.inspection_id),
        "sessionId": str(result.session_id),
        "manifestSha256": manifest.sha256,
        "verificationResultId": str(result.id),
        "verification": verification_payload,
        "issuedAt": issued_at,
        "signing": {"algorithm": "Ed25519", "keyId": key.key_id},
    }
    ...
    if signature.key_id != key.key_id or signature.algorithm != key.algorithm:
        raise SiteProofError(
            ...
            "Signing provider metadata changed during issuance.",
        )
    receipt = SignedReceipt(
        ...
        receipt_type=ReceiptType.AUTOMATED_VERIFICATION,
        ...
        lifecycle_status=ReceiptLifecycleStatus.ISSUED,
        process_status=ReceiptProcessStatus.ISSUED,
        issued_at=issued_at,
    )
    db.add(receipt)
    db.flush()
    previous = list(
        db.scalars(
            select(SignedReceipt).where(
                SignedReceipt.session_id == result.session_id,
                SignedReceipt.receipt_type == ReceiptType.AUTOMATED_VERIFICATION,
                SignedReceipt.id != receipt.id,
                SignedReceipt.lifecycle_status == ReceiptLifecycleStatus.ISSUED,
            )
        ).all()
    )
    for old in previous:
        ...
        record_audit(
            ...
            organization_id=result.organization_id,
            actor_user_id=actor,
            entity_type="SIGNED_RECEIPT",
            entity_id=old.id,
            action="RECEIPT_SUPERSEDED",
            metadata={"supersededBy": str(receipt.id)},
        )
    audit_metadata = {
        ...
        "keyId": receipt.signing_key_id,
    }
    if autonomous_binding is not None:
        audit_metadata["autonomousBindingSha256"] = autonomous_binding["bindingSha256"]
    record_audit(
        db,
        organization_id=result.organization_id,
        actor_user_id=actor,
...
def deep_verify_receipt_evidence(
    db: Session,
    receipt: SignedReceipt,
    *,
    storage: StorageService | None = None,
    ...
    record_audit(
        ...
        entity_type="SIGNED_RECEIPT",
        entity_id=receipt.id,
        action="EVIDENCE_HASH_VERIFIED" if state == "MATCH" else "EVIDENCE_HASH_MISMATCH",
...
def revoke_receipt(
    ...
    *,
    ...
    record_audit(
        ...
        metadata={"reason": receipt.revocation_reason},
    )
    db.commit()
    db.refresh(receipt)
    return receipt
