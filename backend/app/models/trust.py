...
class VerificationPolicy(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "organization_id", "name", "version", name="uq_verification_policy_org_name_version"
        ),
        Index("ix_verification_policy_org_active", "organization_id", "active"),
    )
    ...
    verified_threshold: Mapped[float] = mapped_column(Float, default=85.0, nullable=False)
    review_threshold: Mapped[float] = mapped_column(Float, default=65.0, nullable=False)
    minimum_required_confidence: Mapped[float] = mapped_column(Float, default=0.70, nullable=False)
    ...
    hard_rules_json: Mapped[list[str]] = mapped_column(JSON, nullable=False)
...
class VerificationResult(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_verification_result_session_status", "session_id", "processing_status"),
        Index("ix_verification_result_inspection_verdict", "inspection_id", "verdict"),
    )
    ...
    policy_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, ForeignKey("verification_policies.id", ondelete="RESTRICT"), nullable=False
    )
    ...
    verdict: Mapped[VerificationVerdict | None] = mapped_column(
        Enum(VerificationVerdict, native_enum=False, length=24), nullable=True
    )
...
class VerificationSignalResult(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "verification_result_id", "signal_type", name="uq_verification_signal_result_type"
        ),
    )
    ...
    signal_type: Mapped[VerificationSignalType] = mapped_column(
        Enum(VerificationSignalType, native_enum=False, length=40), index=True, nullable=False
    )
    status: Mapped[VerificationSignalStatus] = mapped_column(
        Enum(VerificationSignalStatus, native_enum=False, length=20), nullable=False
    )
    ...
    source_algorithm_version: Mapped[str | None] = mapped_column(String(64), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
...
class ReviewDecision(Base):
    ...
    session_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, ForeignKey("verification_sessions.id", ondelete="RESTRICT"), index=True, nullable=False
    )
    verification_result_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, ForeignKey("verification_results.id", ondelete="RESTRICT"), index=True, nullable=False
    )
    ...
    decision: Mapped[ReviewDecisionType] = mapped_column(
        Enum(ReviewDecisionType, native_enum=False, length=24), index=True, nullable=False
    )
