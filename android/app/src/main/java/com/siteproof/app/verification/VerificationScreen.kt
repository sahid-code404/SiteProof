...
fun VerificationScreen(viewModel: VerificationViewModel, onBack: () -> Unit) {
    ...
    if (showAbortDialog) {
        AlertDialog(
            ...
            confirmButton = {
                TextButton(onClick = { showAbortDialog = false; viewModel.abortByUser() }) {
                    Text("Stop capture")
                }
            },
            dismissButton = {
            ...
            if (prepared != null) {
                key(prepared.session.sessionId) {
                    PersistentCameraHost(
                        ...
                        retrySemanticConnection = viewModel::retrySemanticChallengeConnection,
                        retryChallenge = viewModel::retryChallenge,
                        completeSemanticChallenge = viewModel::completeSemanticChallenge,
                ...
                when (current) {
                    ...
                    is VerificationUiState.Captured -> CaptureResult(
                        current,
...
private fun cameraPrepared(state: VerificationUiState): VerificationCaptureCoordinator.Prepared? = when (state) {
    ...
    is VerificationUiState.SemanticChallengeLoading -> state.prepared
    is VerificationUiState.SemanticChallengeActive -> state.prepared
    is VerificationUiState.SemanticChallengeChecking -> state.prepared
    is VerificationUiState.SemanticChallengeNetworkWait -> state.prepared
    is VerificationUiState.SemanticChallengeResultState -> state.prepared
...
private fun liveElapsedMs(state: VerificationUiState): Long? = when (state) {
    ...
    is VerificationUiState.SemanticChallengeLoading -> state.elapsedMs
    is VerificationUiState.SemanticChallengeActive -> state.elapsedMs
    is VerificationUiState.SemanticChallengeChecking -> state.elapsedMs
    is VerificationUiState.SemanticChallengeNetworkWait -> state.elapsedMs
    is VerificationUiState.SemanticChallengeResultState -> state.elapsedMs
...
private fun PersistentCameraHost(
    ...
    retrySemanticConnection: () -> Unit,
    retryChallenge: () -> Unit,
    completeSemanticChallenge: () -> Unit,
    onAbort: () -> Unit,
) {
    Box(Modifier.fillMaxSize().background(Color.Black)) {
        ...
        CaptureHeader(
            ...
            state !is VerificationUiState.Ready,
            state is VerificationUiState.StartingCapture,
            requiredRemainingMs(state, requiredSeconds),
        )
        when (state) {
            is VerificationUiState.Ready -> ReadyOverlay(prepared, onStart, onBack)
            is VerificationUiState.StartingCapture ->
                LiveLoadingOverlay("Locking location and starting camera…", onAbort)
            is VerificationUiState.ChallengeLoading ->
                LiveLoadingOverlay("Getting the next movement…", onAbort)
            is VerificationUiState.ChallengeActive -> ChallengeActiveOverlay(state, onAbort)
            is VerificationUiState.ChallengeChecking ->
                LiveLoadingOverlay("Checking that movement…", onAbort)
            is VerificationUiState.ChallengeNetworkWait ->
                NetworkWaitOverlay(state, retryConnection, onAbort)
            is VerificationUiState.ChallengeResultState ->
                ChallengeResultOverlay(state.result, retryChallenge, onAbort)
            is VerificationUiState.SemanticChallengeLoading ->
                LiveLoadingOverlay("Getting an unpredictable visual proof step…", onAbort)
            is VerificationUiState.SemanticChallengeActive ->
                SemanticChallengeActiveOverlay(state, completeSemanticChallenge, onAbort)
            is VerificationUiState.SemanticChallengeChecking ->
                LiveLoadingOverlay("Sealing this visual proof window…", onAbort)
            is VerificationUiState.SemanticChallengeNetworkWait ->
                SemanticNetworkWaitOverlay(state, retrySemanticConnection, onAbort)
            is VerificationUiState.SemanticChallengeResultState ->
                SemanticChallengeResultOverlay(state, onAbort)
    ...
    IndustrialOverlay(
        ...
        Row(
            ...
            if (isRecording && !isStarting) {
                Box(Modifier.size(9.dp).background(RecordingRed, CircleShape))
            }
            Column(Modifier.weight(1f)) {
                Text(
                    when {
                        isStarting -> "STARTING CAPTURE"
            ...
            if (isRecording && !isStarting && recordingRemaining != null) {
                Surface(
                    ...
                    border = BorderStroke(1.dp, OverlayLine),
                ) {
                    Text(
    ...
    Box(
        ...
        Surface(
            ...
            Column(Modifier.padding(21.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                Text(
                    "Camera, microphone, location and motion data are recorded together so the result can be independently reviewed.",
                    color = MaterialTheme.colorScheme.onSurfaceVariant,
                )
                PermissionLine(
                    "Camera",
                    "Records the assigned site while movement and visual proof instructions appear over the preview.",
                )
...
private fun BoxScope.ReadyOverlay(
    ...
    IndustrialOverlay(Modifier.align(Alignment.BottomCenter).fillMaxWidth().padding(10.dp)) {
        Column(
            ...
            StatusRow(
                "Location",
                "${prepared.location.accuracyLabel} · ±${prepared.location.location.accuracyMeters.roundToInt()} m",
            )
            StatusRow(
                "Site distance",
                "${prepared.location.distanceMeters.roundToInt()} m of ${prepared.session.allowedRadiusMeters} m",
            )
            StatusRow("Motion sensors", if (sensorsReady) "Ready" else "Limited")
            StatusRow(
                "Required video",
                "${durationLabel(prepared.session.requiredCaptureDurationSeconds)} minimum",
            )
            if (prepared.session.semanticChallengeCount > 0) {
                StatusRow(
                    "Visual proof steps",
                    "${prepared.session.semanticChallengeCount} unpredictable",
                )
            }
            Text(
                if (prepared.session.semanticChallengeCount > 0) {
                    "Recording stays active while movement checks and task-specific visual proof instructions appear over the camera."
                } else {
                    "Recording stays active while movement instructions appear on top of the camera."
                },
                style = MaterialTheme.typography.bodySmall,
            ...
            TextButton(onClick = onBack, modifier = Modifier.align(Alignment.CenterHorizontally)) {
                Text("Back", color = Color(0xFFCDD2D8))
            }
        }
    }
}
...
@Composable
private fun BoxScope.SemanticChallengeActiveOverlay(
    state: VerificationUiState.SemanticChallengeActive,
    complete: () -> Unit,
    onAbort: () -> Unit,
) {
    IndustrialOverlay(
        Modifier.align(Alignment.BottomCenter).fillMaxWidth().padding(10.dp),
        borderColor = MaterialTheme.colorScheme.primary.copy(alpha = .62f),
    ) {
        Column(
            Modifier.fillMaxWidth().padding(16.dp),
            horizontalAlignment = Alignment.CenterHorizontally,
            ...
            Text(
                if (state.canComplete) {
                    "When the requested evidence is clearly visible, seal this proof window."
                } else {
                    "Keep the requested evidence clearly in frame for a moment…"
                },
                style = MaterialTheme.typography.bodySmall,
                color = Color(0xFFB9C0C9),
                textAlign = TextAlign.Center,
            ...
            Text(
                "${secondsLabel(state.remainingMs)} remaining · recording must stay continuous",
                style = MaterialTheme.typography.bodySmall,
                color = Color(0xFFADB4BD),
            )
            Button(
                ...
                Text(if (state.canComplete) "Done — proof shown" else "Hold evidence in view")
            }
            TextButton(onClick = onAbort) { Text("Stop capture", color = Color(0xFFB9C0C9)) }
        }
    }
...
private fun BoxScope.NetworkWaitOverlay(
    ...
    IndustrialOverlay(
        Modifier.align(Alignment.BottomCenter).fillMaxWidth().padding(10.dp),
        borderColor = RecoveryAmber.copy(alpha = .62f),
    ) {
        Column(
            Modifier.fillMaxWidth().padding(16.dp),
            horizontalAlignment = Alignment.CenterHorizontally,
            ...
            Text(
                "Evidence recording is still active. Reconnect and continue without restarting.",
                style = MaterialTheme.typography.bodySmall,
                color = Color(0xFFADB4BD),
                textAlign = TextAlign.Center,
            )
...
@Composable
private fun BoxScope.SemanticNetworkWaitOverlay(
    state: VerificationUiState.SemanticChallengeNetworkWait,
    retry: () -> Unit,
    onAbort: () -> Unit,
) {
    IndustrialOverlay(
        ...
        Column(
            ...
            Text(
                "Visual proof connection interrupted",
                style = MaterialTheme.typography.titleLarge,
                color = RecoveryAmber,
                textAlign = TextAlign.Center,
            )
            ...
            Text(
                "The camera is still recording continuously. Reconnect before the server challenge expires.",
                style = MaterialTheme.typography.bodySmall,
                color = Color(0xFFADB4BD),
                textAlign = TextAlign.Center,
            )
...
private fun BoxScope.ChallengeResultOverlay(
    ...
    val title = when {
        ...
        else -> "Movement inconclusive"
    }
    val detail = when {
        passed -> "The next proof step will appear automatically while recording continues."
        result.retryAllowed -> "Recording is safe. Retry this movement with a fresh challenge."
        else -> "This step cannot be retried. Stop and restart verification from the inspection."
    }
    IndustrialOverlay(
        Modifier.align(Alignment.BottomCenter).fillMaxWidth().padding(10.dp),
        borderColor = if (passed) {
            MaterialTheme.colorScheme.primary.copy(alpha = .6f)
        } else {
            MaterialTheme.colorScheme.error.copy(alpha = .65f)
        },
    ) {
        Column(
            Modifier.fillMaxWidth().padding(16.dp),
...
@Composable
private fun BoxScope.SemanticChallengeResultOverlay(
    state: VerificationUiState.SemanticChallengeResultState,
    onAbort: () -> Unit,
) {
    IndustrialOverlay(
        Modifier.align(Alignment.BottomCenter).fillMaxWidth().padding(10.dp),
        borderColor = MaterialTheme.colorScheme.primary.copy(alpha = .6f),
    ) {
        Column(
            Modifier.fillMaxWidth().padding(16.dp),
            horizontalAlignment = Alignment.CenterHorizontally,
            ...
            Text(
                "Visual proof recorded",
                style = MaterialTheme.typography.titleLarge,
                color = MaterialTheme.colorScheme.primary,
            )
            Text(
                if (state.result.sequenceComplete) {
                    "All required visual proof windows are sealed."
                } else {
                    "This proof window is sealed. The next unpredictable instruction is loading."
                },
                color = Color(0xFFC4CAD1),
                textAlign = TextAlign.Center,
            )
        ...
        Column(
            ...
            Text(
                if (state.remainingMs > 0L) {
                    "Keep the site steady · ${secondsLabel(state.remainingMs)} minimum recording remaining"
                } else {
                    "Minimum recording reached. Sealing evidence…"
                },
                textAlign = TextAlign.Center,
                color = Color(0xFFB9C0C9),
            )
...
private fun CaptureResult(
    state: VerificationUiState.Captured,
    retry: (String) -> Unit,
    onBack: () -> Unit,
) {
    Box(Modifier.fillMaxSize().padding(18.dp), contentAlignment = Alignment.Center) {
        Surface(
            ...
            Column(
                Modifier.fillMaxWidth().padding(21.dp),
                ...
                if (state.uploadStatus == "FAILED") {
                    Surface(
                        ...
                        Text(
                            "Upload failed, but evidence remains safely queued on this device.",
                            Modifier.padding(12.dp),
                            color = MaterialTheme.colorScheme.onErrorContainer,
                        )
                    }
                    Button(
                        onClick = { retry(state.sessionId) },
                        modifier = Modifier.fillMaxWidth(),
                        shape = RoundedCornerShape(14.dp),
                    ) { Text("Retry upload") }
                }
...
private fun friendlyError(message: String): Pair<String, String> {
    ...
    return when {
        "permission" in normalized || "camera" in normalized && "denied" in normalized ||
            "microphone" in normalized ->
            "Capture permissions are needed" to
                "Allow camera, microphone and precise location access, then retry."
        "location" in normalized || "gps" in normalized ->
            "Location could not be confirmed" to
                "Enable location services, move to an open area and try again."
        "sensor" in normalized || "gyro" in normalized || "accelerometer" in normalized ->
            "Motion sensing is unavailable" to
                "Keep the phone steady and retry. This device may not support the required challenge if the error continues."
        "network" in normalized || "connection" in normalized || "timeout" in normalized ->
            "Connection unavailable" to
                "Check the connection. Evidence is retained whenever the workflow can safely retry."
        else -> "Verification could not continue" to
            "Nothing was submitted incorrectly. Retry or return to the inspection."
    }
}
